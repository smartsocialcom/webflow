if (!window.studentDashboardScriptExecuted) {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    const renderMonthlyLogins = loginLog => {
      ...
      // Chart.js paints into the canvas bitmap, so nothing mutates the
      // DOM here for the observer to catch.
      window.ssShimmer.release("studentLoginsPerMonthChartWrapper");
    ...
    const renderBuildingLogins = (loginLog, schoolBuildings) => {
      ...
      // Chart.js paints into the canvas bitmap, so nothing mutates the
      // DOM here for the observer to catch.
      window.ssShimmer.release("studentLoginsPerBuildingWrapper");
      new window.Chart(canvas, {
        type: "doughnut",
    ...
    const renderTopLessons = lessonsLog => {
      ...
      // Chart.js paints into the canvas bitmap, so nothing mutates the
      // DOM here for the observer to catch.
      window.ssShimmer.release("topVisitedLessonsWrapper");
      new window.Chart(canvas, {
        type: "bar",
        data: {
    ...
    // Every element this page fills from Xano. The chart wrappers are
    // claimed rather than the canvases themselves — a <canvas> can't host
    // an overlay, and Chart.js paints into the bitmap without touching the
    // DOM, so those three are released explicitly once they render.
    window.ssShimmer.claim({
      org_name: "line:16",
      time: "line:22",
      student_pin_list: "table:5",
      studentLoginsPerMonthChartWrapper: "chart",
      studentLoginsPerBuildingWrapper: "chart",
      topVisitedLessonsWrapper: "chart"
    });
